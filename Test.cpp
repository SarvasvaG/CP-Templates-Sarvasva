...
#define SpeedIsNeeded ios_base::sync_with_stdio(false); cin.tie(NULL);cout.tie(NULL)

#ifndef ONLINE_JUDGE
#define debug(x) cerr << __LINE__ << ": " << #x << " "; _print(x);cerr << endl;
#define custom_solve(t, temp) cerr << "Test " << temp - t << ":" << endl; solve();cerr << endl;
...
void printD(ld a, int prec, bool bankerRound){
    ...
    if(bankerRound){
        ld temp=floor(a*num);
        if((ll)temp%2==0 && a*num-temp==0.5) cout<<temp/num<<endl;
        else cout<<round(a*num)/num<<endl;
    }
    else{
